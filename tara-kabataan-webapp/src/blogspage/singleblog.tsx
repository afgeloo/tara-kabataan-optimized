...
// IMPORT THE CACHE AND S3 UTILITY DIRECTLY FROM THE MAIN BLOG FILE
import { Blog, _globalBlogsCache, getSafeBlogImageUrl } from "./blogspage";
...
export default function SingleBlog() {
  ...
  // CHECK CACHE FIRST: Zero-latency loading!
  const cachedBlog = useMemo(() => _globalBlogsCache?.find(b => b.blog_id === id) || null, [id]);
  ...
  /* ---------- fetch main blog (Only if Cache Missed) ---------- */
  useEffect(() => {
    if (!id || cachedBlog) return; 

    const ctrl = new AbortController();
    setLoading(true);
    setNotFound(false);

    fetch(`${API_BASE}/blogs.php?blog_id=${id}`, {
      signal: ctrl.signal,
      headers: { Accept: "application/json" } // Removed no-store
    })
      .then((res) => res.json())
      .then((data) => {
        if (data?.blog_id) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/get_blog_images.php?blog_id=${id}`, {
      signal: ctrl.signal,
      ...
      .then(({ success, images }) => {
        if (success && Array.isArray(images)) {
          const resolved = images.map((p: string) => getSafeBlogImageUrl(p));
  ...
  const copyBlogLink = useCallback(async () => {
    ...
    try { await navigator.clipboard.writeText(link); toast.success("Link copied!"); } 
      ...
      const ta = document.createElement("textarea"); ta.value = link; document.body.appendChild(ta); ta.select(); document.execCommand("copy"); document.body.removeChild(ta); toast.success("Link copied!");
    }
  }, []);
    ...
    <div className="single-blog-container">
      ...
      <main className="single-blog-main">
        <button
          ...
          onClick={() => { sessionStorage.setItem("singleBlogScrollY", window.scrollY.toString()); navigate(-1); }}
        >
          ← Go Back
        </button>

        <div className="single-blog-image-wrapper">
          <img src={heroUrl} alt={blog.title} className="single-blog-image" loading="lazy" decoding="async" style={{ cursor: "zoom-in" }} onClick={openHero} />
        </div>
            ...
            {previewImages.map((img, i) => {
              ...
              return (
                <div key={i} className="blog-image-preview">
                  <img src={img} alt={`More ${i}`} loading="lazy" decoding="async" style={{ cursor: "zoom-in" }} onClick={() => openFull(img)} />
                  {isLast && (
        ...
        <div className="single-blog-info">
          ...
          <div className="single-blog-content" dangerouslySetInnerHTML={{ __html: formattedContent }} />
        </div>
              ...
              <div className="blog-gallery-grid">
                {moreImages.map((img, idx) => (
                  <div key={idx} className="blog-gallery-thumb">
                    <img src={img} alt={`Gallery ${idx}`} loading="lazy" decoding="async" onClick={() => openFull(img)} />
                  </div>
                ))}
              </div>
            </div>
      ...
      <ToastContainer position="top-center" autoClose={1500} hideProgressBar closeOnClick pauseOnFocusLoss={false} pauseOnHover={false} limit={1} />
      <Footer />
    </div>
  );
}