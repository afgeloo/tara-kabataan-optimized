...
import { _globalBlogsCache, getSafeBlogImageUrl } from "./blogspage";
...
export default function SingleBlog() {
    ...
    const [blog, setBlog] = useState(cachedBlog);
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE}/get_blog_images.php?blog_id=${id}`, {
            ...
            if (success && Array.isArray(images)) {
                const resolved = images.map((p) => getSafeBlogImageUrl(p));
    ...
    if (loading || !blog)
        return null;
    return (_jsxs("div", { className: "single-blog-container", children: [_jsx(Header, {}), _jsxs("main", { className: "single-blog-main", children: [_jsx("button", { className: "back-button", onClick: () => { sessionStorage.setItem("singleBlogScrollY", window.scrollY.toString()); navigate(-1); }, children: "\u2190 Go Back" }), _jsx("div", { className: "single-blog-image-wrapper", children: _jsx("img", { src: heroUrl, alt: blog.title, className: "single-blog-image", loading: "lazy", decoding: "async", style: { cursor: "zoom-in" }, onClick: openHero }) }), moreImages.length > 0 && (_jsx("div", { className: "blog-more-image-grid", children: previewImages.map((img, i) => {
