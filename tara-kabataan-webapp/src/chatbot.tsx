...
type Msg = { text: string; type: "self" | "other" };
...
const Chatbot: React.FC = () => {
  ...
  const generateSessionId = useCallback((): string => {
    if ("crypto" in window && (window.crypto as any).randomUUID) {
  ...
  const openElement = useCallback(() => {
    ...
    if (messages.length === 0) {
      ...
      setTimeout(() => {
        displayReply(
          "Mabuhay! Ako si Baby Baka, ang iyong gabay mula sa Tara Kabataan. Paano kita matutulungan sa araw na ito?"
        );
      }, 600);
    }
  }, [messages.length]);

  const closeElement = useCallback((event: React.MouseEvent) => {
  ...
  useEffect(() => {
    ...
    const onDocClick = (e: MouseEvent) => {
      const root = (e.target as HTMLElement)?.closest(".floating-chat");
      if (!root) setIsExpanded(false);
    };
  ...
  const handlePredefinedReplies = useCallback(
    (message: string) => {
      if (isTyping || isDisplayingMessage) return;
  ...
  const displayReply = useCallback((reply: string) => {
    setIsTyping(false);
  ...
  useEffect(() => {
    return () => {
      abortGeminiRef.current?.abort();
      abortSearchRef.current?.abort();
      if (typingRaf.current) cancelAnimationFrame(typingRaf.current);
    };
  }, []);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label="Tara, Usap!"
      aria-expanded={isExpanded}
    >
      <IonIcon icon={chatbubblesOutline} style={{ fontSize: "24px" }} />
      <div
        className={`chat ${isExpanded ? "enter" : ""}`}
        style={{ height: isExpanded ? "500px" : "60px", transition: "height 0.5s ease-out" }}
        onClick={(e) => e.stopPropagation()} // don't bubble to opener
      >
        <div
        ...
        >
          ...
          <button onClick={closeElement} aria-label="Close chat">
            <b className="fa fa-times" aria-hidden="true">
              x
            </b>
          </button>
        </div>
        ...
        <div className="footerchat">
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                if (e.shiftKey) return; // allow linebreak with Shift+Enter
                e.preventDefault();
                sendNewMessage();
              }
            }}
