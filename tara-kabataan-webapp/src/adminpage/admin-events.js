...
import { FaSearch, FaPlus, FaBold, FaItalic, FaUnderline, FaImage, FaListUl, FaLink, FaUndo, FaRedo, FaTimes } from "react-icons/fa";
...
const validatePasswordSafety = (profilePassword, profileEmail) => {
...
const processHtmlLinks = (htmlString) => {
    if (!htmlString)
        return "";
...
const AdminEvents = () => {
    ...
    const updateEventStatusAPI = async (event_id, new_status) => {
    ...
    const applyCrop = async () => {
        try {
            ...
            if (cropMode === "edit" && editableEvent)
    ...
    const handleSendOTP = async () => {
        ...
        if (!profilePhone && !profilePassword)
            return toast.error("Phone or password must be provided.");
        if (profilePassword) {
            if (!oldPassword)
            ...
            try {
                ...
                if ((await prevRes.json()).same)
                    return toast.error("New password must be different from previous.");
            }
            catch {
        ...
        try {
            ...
            toast.update(toastId, {
                render: data.success ? (_jsxs("div", { children: [_jsx("strong", { children: "OTP sent." }), _jsx("div", { style: { fontSize: "0.8rem" }, children: "Check spam folder." })] })) : data.message || "Failed to send OTP.",
    ...
    const handleSave = () => {
        ...
        if (Object.values(newEditMissing).some(Boolean))
            return showTempNotification("Please fill out all required fields marked with *");
        ...
        fetch(`${import.meta.env.VITE_API_BASE_URL}/update_event.php`, {
            ...
            if (data.success) {
                ...
                showTempNotification("Failed to update event.");
        })
    ...
    const handleAddNewEventSave = async () => {
        ...
        if (selectedDate < today)
            return showTempNotification("Event date cannot be in the past!");
        ...
        try {
            ...
            if (data.success && data.event) {
                ...
                showTempNotification("Failed to add new event.");
        }
        catch {
            showTempNotification("An error occurred while adding the event.");
        }
    };
    const confirmSingleDelete = async () => {
        ...
        try {
            ...
            if ((await res.json()).success) {
                ...
                showTempNotification("Failed to delete event.");
        }
        catch {
            showTempNotification("An error occurred while deleting.");
        }
    };
    const handleBulkAction = async (endpoint, payload, onSuccessCallback) => {
        try {
    ...
    const applyBulkStatus = (newStatus) => handleBulkAction("update_bulk_event_status.php", { event_ids: selectedEventIds, new_status: newStatus }, () => setEvents((prev) => prev.map((event) => selectedEventIds.includes(event.event_id) ? { ...event, event_status: newStatus } : event)));
    const isFieldLocked = selectedEvent?.event_status === "COMPLETED" || selectedEvent?.event_status === "CANCELLED";
    const renderFormatToolbar = (editorId, setEventState, isAdding) => (_jsxs("div", { className: "admin-blogs-content-image-tools", children: [[{ cmd: "undo", icon: _jsx(FaUndo, {}) }, { cmd: "redo", icon: _jsx(FaRedo, {}) }, { cmd: "bold", icon: _jsx(FaBold, {}) }, { cmd: "italic", icon: _jsx(FaItalic, {}) }, { cmd: "underline", icon: _jsx(FaUnderline, {}) }].map(btn => (_jsx("button", { className: `format-btn ${btn.cmd}`, onMouseDown: (e) => { e.preventDefault(); saveSelection(); }, onClick: () => applyFormatting(btn.cmd), children: btn.icon }, btn.cmd))), _jsx("button", { className: "format-btn bullet", onMouseDown: (e) => { e.preventDefault(); saveSelection(); }, onClick: () => applyFormatting("insertUnorderedList"), children: _jsx(FaListUl, {}) }), _jsx("button", { className: "format-btn link", onMouseDown: (e) => { e.preventDefault(); saveSelection(); }, onClick: () => {
                    ...
                    if (url) {
                        ...
                        if (div) {
                            ...
                            setEventState((prev) => prev ? { ...prev, content: div.innerHTML } : prev);
                        }
                    }
                }, children: _jsx(FaLink, {}) }), _jsx("button", { className: "format-btn image", onMouseDown: (e) => e.preventDefault(), onClick: () => document.getElementById(isAdding ? "add-new-content-image-input" : "edit-content-image-input")?.click(), children: _jsx(FaImage, {}) }), _jsx("input", { type: "file", accept: "image/*", id: isAdding ? "add-new-content-image-input" : "edit-content-image-input", style: { display: "none" }, onChange: async (e) => {
                    const file = e.target.files?.[0];
                    if (!file)
                        return;
                    if (file.size > MAX_FILE_SIZE_BYTES) {
                    ...
                    try {
                        ...
                        if (data.success && data.image_url) {
                            ...
                            if (div) {
                                div.innerHTML += img;
                                setEventState((prev) => ({ ...prev, content: div.innerHTML }));
                            }
                        }
                        else
    ...
    return (_jsxs("div", { className: "admin-events", children: [showCropper && (_jsx("div", { className: "cropper-overlay", children: _jsxs("div", { className: "cropper-container", children: [_jsx("button", { className: "cropper-close-btn", onClick: () => setShowCropper(false), children: _jsx(FaTimes, { size: 20 }) }), _jsx(Cropper, { image: cropSrc, crop: crop, zoom: zoom, aspect: 16 / 9, onCropChange: setCrop, onZoomChange: setZoom, onCropComplete: (_, area) => setCroppedArea(area) }), _jsx("button", { className: "cropper-confirm-btn", onClick: applyCrop, children: "Confirm" })] }) })), _jsxs("div", { className: "admin-events-header", children: [_jsxs("div", { className: "admin-events-search-container", children: [_jsx(FaSearch, { className: "admin-events-search-icon" }), _jsx("input", { type: "text", style: { display: "none" } }), _jsx("input", { type: "password", style: { display: "none" } }), _jsx("input", { type: "text", placeholder: "Search", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), autoComplete: "off", name: "search-blog", id: "search-blog" })] }), _jsxs("div", { className: "admin-events-header-right", children: [_jsxs("div", { className: "admin-blogs-userinfo", onClick: () => setShowProfileModal(true), style: { cursor: "pointer" }, children: [_jsx("div", { className: "userinfo-label", children: "Logged in as:" }), _jsxs("div", { className: "userinfo-details", children: [_jsx("p", { className: "userinfo-name", children: loggedInUser?.user_name || "Admin" }), _jsx("p", { className: "userinfo-email", children: loggedInUser?.user_email || "" })] })] }), showProfileModal && (_jsx("div", { className: "admin-profile-modal", children: _jsxs("div", { className: "admin-profile-modal-box", children: [_jsx("div", { className: "modal-close-icon", onClick: () => { setShowProfileModal(false); resetProfileModal(); }, children: _jsx(FaTimes, {}) }), _jsx("h2", { children: "Change Password" }), _jsx("label", { children: "Email:" }), _jsx("input", { type: "email", value: profileEmail, disabled: true }), isEditingProfile && (_jsxs("div", { style: { position: "relative" }, children: [_jsx("label", { children: "Old Password:" }), _jsx("input", { type: "password", placeholder: "Enter your current password", value: oldPassword, onChange: (e) => setOldPassword(e.target.value), style: { width: "100%" }, required: true }), _jsx("label", { children: "New Password:" }), _jsx("input", { type: showPassword ? "text" : "password", placeholder: "Enter a New Password", value: profilePassword, onChange: (e) => setProfilePassword(e.target.value), style: { width: "100%" } })] })), _jsx("div", { className: "admin-profile-buttons", children: !isEditingProfile ? (_jsx("button", { onClick: () => setIsEditingProfile(true), children: "Edit" })) : (_jsxs(_Fragment, { children: [_jsx("button", { onClick: () => { handleSendOTP(); setOtpRequired(true); }, children: "Send OTP" }), _jsx("button", { onClick: () => { setShowProfileModal(false); resetProfileModal(); }, children: "Cancel" })] })) }), otpSent && (_jsxs("div", { className: "otp-verification", children: [_jsx("label", { children: "Enter 6-digit OTP:" }), _jsx("div", { className: "otp-inputs", children: Array(6).fill("").map((_, index) => (_jsx("input", { ref: (el) => { otpRefs.current[index] = el; }, type: "text", maxLength: 1, className: "otp-box", value: otpInput[index] || "", onChange: (e) => {
                                                        ...
                                                        } }, index))) }), _jsx("button", { onClick: handleVerifyOTP, children: "Verify OTP & Save" })] }))] }) }))] })] }), _jsxs("div", { className: "admin-events-lower-header", children: [_jsxs("div", { className: "admin-events-lower-header-left", children: [_jsx("h1", { children: "Events" }), viewMode === "table" ? (_jsx("div", { className: "admin-events-lower-header-select", children: _jsxs("button", { onClick: () => { setSelectMode(!selectMode); setSelectedEventIds([]); }, children: [_jsx("img", { src: select, className: "admin-blogs-lower-header-select-img" }), " ", selectMode ? "Cancel" : "Select"] }) })) : (_jsxs("div", { className: "admin-blogs-lower-header-show", children: [_jsx("p", { children: "Category" }), _jsxs("div", { className: "admin-blogs-lower-header-category", onClick: () => setOpenCategory(!openCategory), children: [selectedCategory, " ", _jsx("span", { className: "dropdown-arrow", children: "\u25BE" }), openCategory && (_jsx("div", { className: "admin-events-dropdown-menu", children: FILTER_CATEGORIES.map((item) => (_jsx("div", { className: "admin-events-dropdown-item", onClick: () => { setSelectedCategory(item); setOpenCategory(false); }, children: item }, item))) }))] })] }))] }), _jsx("div", { className: "admin-events-lower-header-right", children: _jsxs("div", { className: "admin-blogs-toggle-newblog", children: [_jsxs("div", { className: "admin-blogs-toggle-wrapper", children: [_jsx("button", { className: `admin-blogs-toggle-button ${viewMode === "table" ? "active" : ""}`, onClick: () => setViewMode("table"), children: "Table View" }), _jsx("button", { className: `admin-blogs-toggle-button ${viewMode === "grid" ? "active" : ""}`, onClick: () => setViewMode("grid"), children: "Grid View" })] }), _jsx("div", { className: "admin-events-lower-header-new-event", children: _jsxs("button", { onClick: () => setIsAddingNew(true), children: [_jsx(FaPlus, { className: "admin-icon-left" }), " Add New Event"] }) })] }) })] }), selectMode && (_jsx("div", { className: "admin-events-bulk-actions", children: _jsx("button", { className: "bulk-delete-btn", onClick: () => { setBulkActionType("delete"); setBulkConfirmVisible(true); }, children: "DELETE" }) })), viewMode === "table" ? (_jsxs("div", { className: "admin-events-main-content", children: [_jsx("div", { className: "admin-events-scrollable-table", children: _jsxs("table", { className: "admin-events-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "ID" }), _jsx("th", { children: _jsxs("div", { className: "admin-events-dropdown-trigger", onClick: () => setOpenCategory(!openCategory), children: ["Category ", _jsx("span", { className: "admin-header-dropdown-arrow", children: "\u25BE" }), openCategory && (_jsx("div", { className: "admin-header-dropdown-menu", children: FILTER_CATEGORIES.map((item) => (_jsx("div", { className: "admin-header-dropdown-item", onClick: () => { setSelectedCategory(item); setOpenCategory(false); }, children: item }, item))) }))] }) }), _jsx("th", { children: "Title" }), _jsx("th", { children: _jsxs("div", { className: "admin-events-dropdown-trigger", onClick: () => setOpenCreatedAt(!openCreatedAt), children: ["Date ", _jsx("span", { className: "admin-header-dropdown-arrow", children: "\u25BE" }), openCreatedAt && (_jsx("div", { className: "admin-header-dropdown-menu", children: ["Newest First", "Oldest First"].map((order) => (_jsx("div", { className: "admin-header-dropdown-item", onClick: () => { setCreatedSortOrder(order); setOpenCreatedAt(false); }, children: order }, order))) }))] }) }), _jsx("th", { children: "Venue" }), _jsx("th", { children: _jsxs("div", { className: "admin-events-dropdown-trigger", onClick: () => setOpenStatus(!openStatus), children: ["Status ", _jsx("span", { className: "admin-header-dropdown-arrow", children: "\u25BE" }), openStatus && (_jsx("div", { className: "admin-header-dropdown-menu", children: ["All", "Upcoming", "Ongoing", "Completed", "Cancelled"].map((status) => (_jsx("div", { className: "admin-header-dropdown-item", onClick: () => { setSelectedStatus(status); setOpenStatus(false); }, children: status }, status))) }))] }) }), _jsx("th", { children: selectMode ? "Select" : "View" })] }) }), _jsxs("colgroup", { children: [_jsx("col", { style: { width: "80px" } }), _jsx("col", { style: { width: "70px" } }), _jsx("col", { style: { width: "80px" } }), _jsx("col", { style: { width: "70px" } }), _jsx("col", { style: { width: "80px" } }), _jsx("col", { style: { width: "60px" } }), _jsx("col", { style: { width: "40px" } })] }), _jsx("tbody", { children: loading ? (_jsx("tr", { children: _jsxs("td", { colSpan: 7, className: "no-blogs-message", children: [_jsx("span", { className: "loading-spinner" }), " Loading events\u2026"] }) })) : pagedEvents.length > 0 ? (pagedEvents.map((event) => (_jsxs("tr", { className: "admin-events-table-content", style: { cursor: selectMode ? "default" : "pointer" }, onClick: () => { if (!selectMode)
                                            setSelectedEvent(event); }, children: [_jsx("td", { className: "admin-events-id-content", children: event.event_id }), _jsx("td", { className: "admin-events-category-content category-tag", children: event.category?.toUpperCase() || "UNSPECIFIED" }), _jsx("td", { className: "admin-events-title-content", children: event.title }), _jsx("td", { className: "admin-events-date-content", children: formatDate(event.event_date) }), _jsx("td", { className: "admin-events-venue-content", children: event.event_venue }), _jsx("td", { className: `event-status status-${event.event_status.toLowerCase()}`, children: event.event_status.toUpperCase() }), _jsx("td", { className: "admin-events-more-button", children: selectMode ? (_jsx("input", { type: "checkbox", checked: selectedEventIds.includes(event.event_id), onClick: (e) => e.stopPropagation(), onChange: (e) => setSelectedEventIds((prev) => e.target.checked ? [...prev, event.event_id] : prev.filter((id) => id !== event.event_id)) })) : (_jsx("button", { onClick: (e) => { e.stopPropagation(); setSelectedEvent(event); }, children: _jsx(BsThreeDots, {}) })) })] }, event.event_id)))) : (_jsx("tr", { className: "admin-blogs-table-content no-blogs-row", children: _jsx("td", { colSpan: 7, className: "no-blogs-message", children: "No Event Found." }) })) })] }) }), _jsx("div", { className: "pagination-container", children: totalEventPages > 1 && (_jsxs("div", { className: "pagination", children: [_jsx("button", { onClick: () => setEventsPage((p) => p - 1), disabled: eventsPage === 1, children: "\u2039 Prev" }), [...Array(totalEventPages)].map((_, i) => (_jsx("button", { className: i + 1 === eventsPage ? "active" : "", onClick: () => setEventsPage(i + 1), children: i + 1 }, i + 1))), _jsx("button", { onClick: () => setEventsPage((p) => p + 1), disabled: eventsPage === totalEventPages, children: "Next \u203A" })] })) })] })) : (_jsx("div", { className: "admin-events-main-content", children: _jsx("div", { className: "admin-blogs-grid-view", children: filteredEvents.length > 0 ? (_jsx("div", { className: "blog-grid-scrollable-wrapper", children: _jsx("div", { className: "blog-grid-container", children: filteredEvents.map((event) => (_jsxs("div", { className: `blog-grid-card grid-status-${event.event_status.toLowerCase()}`, onClick: () => setSelectedEvent(event), children: [_jsx("img", { src: getFullImageUrl(event.image_url), alt: event.title, className: "blog-grid-image" }), _jsxs("div", { className: "blog-grid-overlay", children: [_jsx("h3", { className: "blog-overlay-title", children: event.title }), _jsx("p", { className: "blog-overlay-category", children: event.category }), _jsx("p", { className: "blog-overlay-date", children: formatDate(event.event_date) }), _jsx("p", { className: "blog-overlay-venue", children: event.event_venue })] })] }, event.event_id))) }) })) : _jsx("p", { style: { marginTop: "20px" }, children: "No events found." }) }) })), selectedEvent && (_jsx("div", { className: "admin-events-view-more", children: _jsx("div", { className: "admin-events-modal", children: _jsxs("div", { className: "admin-events-modal-content", children: [_jsx("div", { className: "admin-events-float-buttons", children: isEditing ? (_jsxs(_Fragment, { children: [_jsx("button", { className: "save-btn", onClick: handleSave, children: "Save" }), _jsx("button", { className: "cancel-btn", onClick: () => { setIsEditing(false); setEditableEvent(null); }, children: "Cancel" })] })) : (_jsxs(_Fragment, { children: [_jsx("button", { className: "edit-btn", onClick: () => { setEditableEvent({ ...selectedEvent }); setIsEditing(true); setTimeout(() => { if (textareaRef.current)
                                                textareaRef.current.innerHTML = selectedEvent.content || ""; }, 0); }, children: "Edit" }), _jsx("button", { className: "delete-btn", onClick: () => { setBulkActionType("delete"); setBulkActionStatus("SINGLE_DELETE"); setBulkConfirmVisible(true); }, children: "Delete" })] })) }), _jsx("button", { className: "admin-events-modal-close", onClick: () => { setIsEditing(false); setEditableEvent(null); setTempImageUrl(null); setSelectedEvent(null); }, children: "\u2715" }), _jsxs("div", { className: "admin-events-inner-content-modal", children: [notification && _jsx("div", { className: `blogs-notification-message ${notification.includes("successfully") ? "success" : "error"} show`, children: notification }), _jsxs("div", { className: "admin-events-inner-content-modal-top", children: [_jsxs("div", { className: "admin-events-inner-content-modal-top-left", children: [_jsx("h2", { children: "Event Details" }), _jsxs("div", { className: "admin-events-inner-content-modal-id", children: [_jsx("p", { children: _jsx("strong", { children: "ID" }) }), _jsx("p", { className: "admin-events-inner-content-modal-id-content", children: selectedEvent.event_id })] }), _jsxs("div", { className: "admin-events-inner-content-modal-title", children: [_jsx("p", { children: _jsxs("strong", { children: ["Title ", editMissing.title && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), isEditing ? _jsx("input", { type: "text", value: editableEvent?.title || "", onChange: (e) => setEditableEvent({ ...editableEvent, title: e.target.value }), className: "admin-events-inner-content-modal-title-content" }) : _jsx("p", { className: "admin-events-inner-content-modal-title-content", children: selectedEvent.title })] }), _jsxs("div", { className: "admin-events-inner-content-modal-category", children: [_jsx("p", { children: _jsx("strong", { children: "Category" }) }), _jsx("select", { className: "admin-events-inner-content-modal-category-content pink-category", value: dropdownCategory ?? "", onChange: (e) => { setDropdownCategory(e.target.value); setEditableEvent((prev) => prev ? { ...prev, category: e.target.value } : prev); }, disabled: !isEditing, children: CATEGORIES.map((cat) => _jsx("option", { value: cat, children: cat }, cat)) })] }), _jsxs("div", { className: "admin-events-inner-content-modal-venue", children: [_jsx("p", { children: _jsxs("strong", { children: ["Venue ", editMissing.event_venue && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), isEditing ? _jsx("input", { type: "text", value: editableEvent?.event_venue || "", onChange: (e) => setEditableEvent((prev) => prev ? { ...prev, event_venue: e.target.value } : prev), onBlur: () => setEditableEvent((prev) => prev ? { ...prev, event_venue: prev.event_venue.trim() === "" ? "Manila, Philippines" : prev.event_venue } : prev), className: "admin-events-inner-content-modal-venue-content", disabled: isFieldLocked }) : _jsx("p", { className: "admin-events-inner-content-modal-venue-content", children: selectedEvent.event_venue })] }), (isEditing ? editableEvent?.event_venue : selectedEvent.event_venue) && (_jsx("div", { className: "admin-events-google-map", children: _jsx("iframe", { src: `https://www.google.com/maps?q=${encodeURIComponent((isEditing ? editableEvent?.event_venue : selectedEvent.event_venue) ?? "")}&z=18&output=embed`, width: "100%", height: "250", loading: "lazy", style: { border: "0", borderRadius: "10px", marginTop: "15px" }, allowFullScreen: true }) }))] }), _jsxs("div", { className: "admin-events-inner-content-modal-top-right", children: [_jsxs("div", { className: "admin-events-inner-content-modal-status", children: [_jsx("p", { children: _jsx("strong", { children: "Status" }) }), _jsxs("select", { className: `admin-events-inner-content-modal-status-content status-${dropdownStatus?.toLowerCase()}`, value: dropdownStatus ?? "", onChange: (e) => { setDropdownStatus(e.target.value); setEditableEvent((prev) => prev ? { ...prev, event_status: e.target.value } : prev); }, disabled: !isEditing || isFieldLocked, children: [dropdownStatus && !["UPCOMING", "CANCELLED"].includes(dropdownStatus) && _jsx("option", { value: dropdownStatus, disabled: true, children: dropdownStatus }), ["UPCOMING", "CANCELLED"].map((stat) => _jsx("option", { value: stat, children: stat }, stat))] })] }), _jsxs("div", { className: "admin-events-inner-content-modal-date", children: [_jsx("p", { children: _jsx("strong", { children: "Date" }) }), isEditing ? _jsx("input", { type: "date", value: editableEvent?.event_date || "", onChange: (e) => setEditableEvent({ ...editableEvent, event_date: e.target.value }), className: "admin-events-inner-content-modal-date-content", disabled: isFieldLocked }) : _jsx("p", { className: "admin-events-inner-content-modal-date-content", children: formatDate(selectedEvent.event_date) })] }), _jsxs("div", { className: "admin-events-inner-content-modal-time", children: [_jsxs("div", { className: "admin-events-inner-content-modal-time-start", children: [_jsx("p", { children: _jsx("strong", { children: "Start Time" }) }), isEditing ? _jsx("input", { type: "time", value: editableEvent?.event_start_time || "", onChange: (e) => setEditableEvent({ ...editableEvent, event_start_time: e.target.value }), className: "admin-events-inner-content-modal-time-start-content", disabled: isFieldLocked }) : _jsx("p", { className: "admin-events-inner-content-modal-time-start-content", children: formatTime(selectedEvent.event_start_time) })] }), _jsxs("div", { className: "admin-events-inner-content-modal-time-end", children: [_jsx("p", { children: _jsx("strong", { children: "End Time" }) }), isEditing ? _jsx("input", { type: "time", value: editableEvent?.event_end_time || "", onChange: (e) => setEditableEvent({ ...editableEvent, event_end_time: e.target.value }), className: "admin-events-inner-content-modal-time-end-content", disabled: isFieldLocked }) : _jsx("p", { className: "admin-events-inner-content-modal-time-end-content", children: formatTime(selectedEvent.event_end_time) })] })] }), _jsxs("div", { className: "admin-events-inner-content-modal-speakers", children: [_jsx("p", { children: _jsxs("strong", { children: ["Speakers ", editMissing.event_speakers && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), isEditing ? _jsx("textarea", { value: editableEvent?.event_speakers || "", onChange: (e) => setEditableEvent({ ...editableEvent, event_speakers: e.target.value }), className: "admin-events-inner-content-modal-speakers-content" }) : _jsx("p", { className: "admin-events-inner-content-modal-speakers-content", style: { whiteSpace: "pre-wrap" }, children: selectedEvent.event_speakers })] })] })] }), _jsxs("div", { className: "admin-events-inner-content-modal-bot", children: [_jsx("div", { className: "admin-events-inner-content-modal-bot-left", children: _jsxs("div", { className: "admin-events-inner-content-modal-image", children: [_jsx("p", { children: _jsxs("strong", { children: ["Image ", editMissing.image_url && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), getFullImageUrl(isEditing ? tempImageUrl ?? editableEvent?.image_url ?? "" : selectedEvent.image_url) ? (_jsx("img", { src: getFullImageUrl(isEditing ? tempImageUrl ?? editableEvent?.image_url ?? "" : selectedEvent.image_url), alt: "Event", style: { cursor: "zoom-in" }, onClick: () => setFullscreenImageUrl(getFullImageUrl(isEditing ? tempImageUrl ?? editableEvent?.image_url ?? "" : selectedEvent.image_url)) })) : (_jsx("div", { style: { width: "100%", height: "200px", maxWidth: "100%", maxHeight: "300px", backgroundColor: "#f2f2f2", color: "#888", display: "flex", alignItems: "center", justifyContent: "center", fontStyle: "italic", border: "1px dashed #ccc" }, children: "No Event Image" })), _jsx("input", { type: "file", id: "event-image-upload", accept: "image/*", style: { display: "none" }, onChange: (e) => handleImageSelect(e, "edit") }), _jsxs("div", { className: "admin-blogs-image-buttons", children: [_jsx("button", { className: "upload-btn", disabled: !isEditing, onClick: () => document.getElementById("event-image-upload")?.click(), children: "Upload" }), _jsx("button", { className: "remove-btn", disabled: !isEditing, onClick: () => { setTempImageUrl(null); if (editableEvent)
                                                                        setEditableEvent({ ...editableEvent, image_url: "" }); }, children: "Remove" })] })] }) }), _jsx("div", { className: "admin-events-inner-content-modal-bot-right", children: _jsxs("div", { className: "admin-events-inner-content-modal-desc", children: [_jsx("p", { children: _jsxs("strong", { children: ["Event Content ", editMissing.content && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), isEditing ? (_jsxs(_Fragment, { children: [renderFormatToolbar("new-event-content-editor", setEditableEvent, false), _jsx("div", { id: "new-event-content-editor", ref: textareaRef, className: "admin-blogs-modal-desc-content editable", contentEditable: true, onBlur: () => { if (textareaRef.current) {
                                                                        const updatedContent = textareaRef.current.innerHTML;
                                                                        setEditableEvent((prev) => prev ? { ...prev, content: updatedContent } : prev);
                                                                    } } })] })) : (_jsx("div", { className: "admin-blogs-modal-desc-content", children: _jsx("div", { className: "admin-blogs-content-images-wrapper", children: _jsx("div", { dangerouslySetInnerHTML: { __html: selectedEvent.content } }) }) }))] }) })] }), _jsxs("div", { className: "event-participants card", children: [_jsx("h3", { children: "Participants" }), participants.length > 0 ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "participants-table-wrapper", children: _jsxs("table", { className: "participants-table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Name" }), _jsx("th", { children: "Email" }), _jsx("th", { children: "Contact" }), _jsx("th", { children: "Expectations" }), _jsx("th", { children: "Signed Up" })] }) }), _jsx("tbody", { children: paginatedParticipants.map((p) => (_jsxs("tr", { children: [_jsx("td", { children: p.name }), _jsx("td", { children: p.email }), _jsx("td", { children: p.contact || "—" }), _jsx("td", { children: p.expectations || "—" }), _jsx("td", { children: new Date(p.created_at).toLocaleDateString() })] }, p.participant_id))) })] }) }), totalPages > 1 && (_jsxs("div", { className: "pagination", children: [_jsx("button", { onClick: () => setCurrentPage((p) => p - 1), disabled: currentPage === 1, children: "\u2039 Prev" }), [...Array(totalPages)].map((_, i) => (_jsx("button", { className: i + 1 === currentPage ? "active" : "", onClick: () => setCurrentPage(i + 1), children: i + 1 }, i + 1))), _jsx("button", { onClick: () => setCurrentPage((p) => p + 1), disabled: currentPage === totalPages, children: "Next \u203A" })] }))] })) : _jsx("p", { className: "no-participants", children: "No one has signed up yet." })] })] })] }) }) })), bulkConfirmVisible && (_jsx("div", { className: "blogs-confirmation-popup show", children: _jsxs("div", { className: "blogs-confirmation-box", children: [_jsx("p", { children: bulkActionType === "delete" && bulkActionStatus === "SINGLE_DELETE" ? "Are you sure you want to delete this event and all its images?"
                                : bulkActionType === "delete" ? "Are you sure you want to delete the selected events?" : `Do you really want to mark the selected events as ${bulkActionStatus}?` }), _jsxs("div", { className: "blogs-confirmation-actions", children: [_jsx("button", { className: "confirm-yes", onClick: () => { if (bulkActionType === "delete") {
                                        bulkActionStatus === "SINGLE_DELETE" ? confirmSingleDelete() : handleBulkDelete();
                                    }
                                    else {
                                        applyBulkStatus(bulkActionStatus);
                                    } setBulkConfirmVisible(false); }, children: "Yes" }), _jsx("button", { className: "confirm-no", onClick: () => setBulkConfirmVisible(false), children: "No" })] })] }) })), isAddingNew && (_jsx("div", { className: "admin-new-event", children: _jsx("div", { className: "admin-new-event-modal", children: _jsxs("div", { className: "admin-new-event-modal-content", children: [_jsxs("div", { className: "admin-new-event-float-buttons", children: [_jsx("button", { className: "save-btn", onClick: handleAddNewEventSave, children: "Save" }), _jsx("button", { className: "cancel-btn", onClick: () => { setIsAddingNew(false); resetNewEvent(); }, children: "Cancel" })] }), _jsx("button", { className: "admin-new-event-modal-close", onClick: () => { setIsAddingNew(false); resetNewEvent(); }, children: "\u2715" }), _jsxs("div", { className: "admin-new-event-inner-content-modal", children: [notification && _jsx("div", { className: `blogs-notification-message ${notification.includes("successfully") ? "success" : "error"} show`, children: notification }), _jsxs("div", { className: "admin-new-event-inner-content-modal-top", children: [_jsxs("div", { className: "admin-new-event-inner-content-modal-top-left", children: [_jsx("h2", { children: "Event Details" }), _jsx("p", { children: _jsxs("strong", { children: ["Title ", missing.title && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), _jsx("input", { className: "admin-new-event-inner-content-modal-title-content", type: "text", value: newEvent.title, onChange: (e) => setNewEvent({ ...newEvent, title: e.target.value }) }), _jsxs("div", { className: "admin-new-event-inner-content-modal-category", children: [_jsx("p", { children: _jsxs("strong", { children: ["Category ", missing.category && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), _jsx("select", { className: "admin-events-inner-content-modal-category-content pink-category", value: newEvent.category, onChange: (e) => setNewEvent({ ...newEvent, category: e.target.value }), children: CATEGORIES.map((cat) => _jsx("option", { value: cat, children: cat }, cat)) })] }), _jsxs("div", { className: "admin-new-event-inner-content-modal-venue", children: [_jsx("p", { children: _jsxs("strong", { children: ["Venue ", missing.event_venue && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), _jsx("input", { type: "text", className: "admin-new-event-inner-content-modal-venue-content", value: newEvent.event_venue, onChange: (e) => setNewEvent({ ...newEvent, event_venue: e.target.value }) })] }), newEvent.event_venue && (_jsx("div", { className: "admin-events-google-map", children: _jsx("iframe", { src: `https://www.google.com/maps?q=${encodeURIComponent(newEvent.event_venue)}&z=18&output=embed`, width: "100%", height: "250", loading: "lazy", style: { border: "0", borderRadius: "10px", marginTop: "15px" }, allowFullScreen: true }) }))] }), _jsxs("div", { className: "admin-new-event-inner-content-modal-top-right", children: [_jsxs("div", { className: "admin-new-event-inner-content-modal-status", children: [_jsx("p", { children: _jsx("strong", { children: "Status" }) }), _jsx("select", { className: `admin-events-inner-content-modal-status-content status-${newEvent.event_status.toLowerCase()}`, value: newEvent.event_status, disabled: true, children: _jsx("option", { value: "UPCOMING", children: "UPCOMING" }) })] }), _jsxs("div", { className: "admin-new-event-inner-content-modal-date", children: [_jsx("p", { children: _jsxs("strong", { children: ["Date ", missing.event_date && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), _jsx("input", { type: "date", className: "admin-new-event-inner-content-modal-date-content", value: newEvent.event_date, onChange: (e) => setNewEvent({ ...newEvent, event_date: e.target.value }) })] }), _jsxs("div", { className: "admin-new-event-inner-content-modal-time", children: [_jsxs("div", { className: "admin-new-event-inner-content-modal-time-start", children: [_jsx("p", { children: _jsxs("strong", { children: ["Start Time ", missing.event_start_time && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), _jsx("input", { type: "time", className: "admin-new-event-inner-content-modal-time-start-content", value: newEvent.event_start_time, onChange: (e) => setNewEvent({ ...newEvent, event_start_time: e.target.value }) })] }), _jsxs("div", { className: "admin-new-event-inner-content-modal-time-end", children: [_jsx("p", { children: _jsxs("strong", { children: ["End Time ", missing.event_end_time && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), _jsx("input", { type: "time", className: "admin-new-event-inner-content-modal-time-end-content", value: newEvent.event_end_time, onChange: (e) => setNewEvent({ ...newEvent, event_end_time: e.target.value }) })] })] }), _jsxs("div", { className: "admin-events-inner-content-modal-speakers", children: [_jsx("p", { children: _jsxs("strong", { children: ["Speaker/s ", missing.event_speakers && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), _jsx("textarea", { className: "admin-new-event-inner-content-modal-speakers-content", value: newEvent.event_speakers, onChange: (e) => setNewEvent({ ...newEvent, event_speakers: e.target.value }) })] })] })] }), _jsxs("div", { className: "admin-new-event-inner-content-modal-bot", children: [_jsx("div", { className: "admin-new-event-inner-content-modal-bot-left", children: _jsxs("div", { className: "admin-new-event-inner-content-modal-image", children: [_jsx("p", { children: _jsxs("strong", { children: ["Image ", missing.image && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), getFullImageUrl(newImageUrl) ? (_jsx("img", { src: getFullImageUrl(newImageUrl), alt: "Event", style: { cursor: "zoom-in" }, onClick: () => setFullscreenImageUrl(getFullImageUrl(newImageUrl)) })) : (_jsx("div", { style: { width: "100%", height: "200px", maxWidth: "100%", maxHeight: "300px", backgroundColor: "#f2f2f2", color: "#888", display: "flex", alignItems: "center", justifyContent: "center", fontStyle: "italic", border: "1px dashed #ccc" }, children: "No Event Image" })), _jsx("input", { type: "file", accept: "image/*", style: { display: "none" }, id: "new-event-image-upload", onChange: (e) => handleImageSelect(e, "new") }), _jsxs("div", { className: "admin-blogs-image-buttons", children: [_jsx("button", { className: "upload-btn", onClick: () => document.getElementById("new-event-image-upload")?.click(), children: "Upload" }), _jsx("button", { className: "remove-btn", onClick: () => setNewImageUrl(null), children: "Remove" })] })] }) }), _jsx("div", { className: "admin-new-event-inner-content-modal-bot-right", children: _jsxs("div", { className: "admin-new-event-inner-content-modal-desc", children: [_jsx("p", { children: _jsxs("strong", { children: ["Event Content ", missing.content && _jsx("span", { style: { color: "red" }, children: "*" })] }) }), renderFormatToolbar("add-event-content-editor", setNewEvent, true), _jsx("div", { id: "add-event-content-editor", className: "admin-new-event-inner-content-modal-desc-content editable", contentEditable: true, onBlur: () => { const div = document.getElementById("add-event-content-editor"); if (div) {
