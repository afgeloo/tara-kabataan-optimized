...
const AdminBlogs = () => {
  ...
  try {
    ...
    if (data.success && data.image_url) {
      ...
      // CHANGE THIS LINE to see the real error from PHP
    ...
    async (newStatus: string) => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/update_bulk_blog_status.php`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const handleBulkDelete = useCallback(async () => {
    try {
      const res = await fetch(
        `${API_BASE}/delete_bulk_blogs.php`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleNewBlogSave = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.blog) {
        ...
        try {
          const fresh = await fetch(`${API_BASE}/blogs.php`).then(
            (r) => r.json()
          );
