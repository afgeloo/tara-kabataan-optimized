...
const formatDate = (ts: string) => {
  ...
  return Number.isNaN(t) ? ts : DT_FMT.format(new Date(t));
};

// --- ZERO LATENCY MEMORY CACHE ---
// Allows instant loading while fetching fresh data in the background
let _adminBlogsCache: Blog[] | null = null;
...
const AdminBlogs = memo(() => {
  // ------------------------------------
  // Core state (Zero-Latency Init)
  // ------------------------------------
  const [blogs, setBlogs] = useState<Blog[]>(_adminBlogsCache || []);
  const [loading, setLoading] = useState<boolean>(!_adminBlogsCache);
  ...
  const [croppedArea, setCroppedArea] = useState<{ x: number; y: number; width: number; height: number }>({ x: 0, y: 0, width: 0, height: 0 });
  ...
  // ------------------------------------
  // Profile & OTP Handling
  // ------------------------------------
  const handleSendOTP = useCallback(async () => {
    // TODO: Replace with your actual API endpoint to send the OTP
    try {
      /*
      ...
      */
      ...
      showNote("OTP sent to your email/phone!", "success");
      setOtpSent(true);
    } catch (error) {
      console.error("Failed to send OTP:", error);
      showNote("Failed to send OTP.", "error");
    }
  }, [profileEmail, showNote]);

  const handleVerifyOTP = useCallback(async () => {
    if (otpInput.length < 6) {
      showNote("Please enter the full 6-digit OTP.", "error");
      return;
    }

    // TODO: Replace with your actual API endpoint to verify and save the new password
    try {
      /*
      const res = await fetch(`${API_BASE}/verify_otp_and_update_profile.php`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          otp: otpInput, 
      ...
      */
      ...
      showNote("Invalid OTP or failed to update profile.", "error");
    }
  }, [otpInput, oldPassword, profilePassword, loggedInUser, showNote]);

  // ------------------------------------
  // Stale-While-Revalidate Fetch (Instant UI + Background Sync)
  // ------------------------------------
  useEffect(() => {
    ...
    fetch(`${API_BASE}/blogs.php`, {
      ...
      .then((data) => {
        if (data?.blogs && Array.isArray(data.blogs)) {
          const freshBlogs = data.blogs.map((b: Blog) => ({
            ...b,
            more_images: b.more_images ?? [],
          }));
          
          // Update Cache and State silently
          _adminBlogsCache = freshBlogs;
          setBlogs(freshBlogs);
        } else {
          if (!blogs.length) toast.error("Invalid blogs payload.");
        }
      })
      .catch((err) => {
        if (err?.name !== "AbortError") {
          console.error("Failed to fetch blogs:", err);
          if (!blogs.length) toast.error("Unable to load blogs.");
        }
      })
  ...
  useEffect(() => {
    ...
    try { setLoggedInUser(JSON.parse(storedUser)); } 
  ...
  useEffect(() => { setCurrentPage(1); }, [selectedCategory, selectedStatus, createdSortOrder, searchQuery]);
  ...
  // Cache Sync Helpers
  // ------------------------------------
  const syncBlogsState = useCallback((newBlogs: Blog[]) => {
    setBlogs(newBlogs);
    _adminBlogsCache = newBlogs; // Keeps memory perfectly in sync
  }, []);
  ...
  const handleSave = useCallback(() => {
    ...
    const merged: Blog = { ...editableBlog, content: updatedHTML, more_images: editableBlogMoreImages } as Blog;
    ...
    fetch(`${API_BASE}/update_blogs.php`, {
      ...
      .then((data) => {
        if (data.success) {
          syncBlogsState(blogs.map((b) => (b.blog_id === merged.blog_id ? merged : b)));
  ...
  }, [editableBlog, editableBlogMoreImages, showNote, blogs, syncBlogsState]);
  ...
  const applyCrop = useCallback(async () => {
    try {
      ...
      let endpoint = cropMode === "new" ? `${API_BASE}/add_new_blog_image.php` : `${API_BASE}/upload_blog_image.php`;
      if (cropMode === "edit" && editableBlog?.blog_id) form.append("blog_id", editableBlog.blog_id);

      const res = await fetch(endpoint, { method: "POST", body: form });
      const data = await res.json();

      if (data.success && data.image_url) {
        if (cropMode === "new") setNewBlogImage(data.image_url);
        else setEditableBlog((prev) => (prev ? { ...prev, image_url: data.image_url } : prev));
        toast.success("Image uploaded!");
      } else {
        toast.error(data.error || "Upload failed: Check PHP logs");
      }
    } catch (err) {
  ...
  }, [cropSrc, croppedArea, cropMode, editableBlog?.blog_id, API_BASE]);
  ...
  const confirmDeleteBlog = useCallback(() => {
    ...
    fetch(`${API_BASE}/delete_blogs.php`, {
      ...
      .then((data) => {
        if (data.success) {
          syncBlogsState(blogs.filter((b) => b.blog_id !== selectedBlog.blog_id));
  ...
  }, [selectedBlog, showNote, blogs, syncBlogsState]);

  // ------------------------------------
  // Bulk actions
  // ------------------------------------
  const applyBulkStatus = useCallback(async (newStatus: string) => {
    if (newStatus === "PINNED") {
    ...
    try {
      const res = await fetch(`${API_BASE}/update_bulk_blog_status.php`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ blog_ids: selectedBlogIds, new_status: newStatus, more_images: editableBlogMoreImages }),
      });
      const data = await res.json();
      if (data.success) {
        syncBlogsState(blogs.map((b) => (selectedBlogIds.includes(b.blog_id) ? { ...b, blog_status: newStatus } : b)));
        setSelectedBlogIds([]);
        setSelectMode(false);
  ...
  }, [selectedBlogIds, editableBlogMoreImages, blogs, showNote, syncBlogsState]);
  ...
  const handleBulkDelete = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        syncBlogsState(blogs.filter((b) => !selectedBlogIds.includes(b.blog_id)));
        setSelectedBlogIds([]);
        setSelectMode(false);
  ...
  }, [selectedBlogIds, showNote, blogs, syncBlogsState]);
  ...
  const handleNewBlogSave = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.blog) {
        syncBlogsState([data.blog, ...blogs]); // Optimistic cache injection
        resetNewBlogForm();
  ...
  }, [newBlogTitle, newBlogContent, newBlogImage, newBlogCategory, newBlogStatus, newBlogAuthor, newBlogMoreImages, resetNewBlogForm, showNote, blogs, syncBlogsState]);
    ...
    <div className="admin-blogs">
      ...
      <div className="admin-blogs-header">
        <div className="admin-blogs-search-container">
          ...
          <input type="text" placeholder="Search" defaultValue={searchQuery} onChange={(e) => onSearchChange(e.target.value)} autoComplete="off" name="search-blog" id="search-blog" />
        </div>
                ...
                {isEditingProfile && (
                  <div style={{ position: "relative" }}>
                    ...
                    <form autoComplete="off">
                      <input type="password" placeholder="Enter your current password" autoComplete="current-password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} style={{ width: "100%" }} required />
                    </form>

                    <label>New Password:</label>
                    <form autoComplete="off">
                      <input type="password" placeholder="Enter a New Password" autoComplete="new-password" value={profilePassword} readOnly={!isEditingProfile} onChange={(e) => setProfilePassword(e.target.value)} style={{ width: "100%", color: !isEditingProfile ? "#999" : "inherit", cursor: !isEditingProfile ? "default" : "text" }} />
                    </form>
                  </div>
                )}
                ...
                <div className="admin-profile-buttons">
                  {!isEditingProfile ? (
                    ...
                    <>
                      ...
                      <button onClick={() => { setIsEditingProfile(false); setOtpSent(false); setOtpInput(""); setProfilePhone(loggedInUser?.user_contact || ""); setProfilePassword(""); setOldPassword(""); }}>Cancel</button>
                    </>
                  )}
                </div>
              ...
              <div className="admin-blogs-lower-header-category" onClick={() => setOpenCategory((v) => !v)}>
                ...
                {openCategory && (
                  <div className="admin-blogs-dropdown-menu">
                    {CATEGORIES.map((cat) => (
                      <div key={cat} className="admin-blogs-dropdown-item" onClick={() => { setSelectedCategory(cat); setOpenCategory(false); }}>{cat}</div>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className="admin-blogs-lower-header-right">
          <div className="admin-blogs-toggle-newblog">
            ...
            <div className="admin-blogs-lower-header-new-blog">
              <button onClick={() => { resetNewBlogForm(); setNewBlogMoreImages([]); setNewBlogImage(""); setNewBlogContent(""); setShowAllImagesModal(false); setNewBlogModalOpen(true); }}>
                <FaPlus className="admin-icon-left" /> Add New Blog
              </button>
            </div>
          </div>
        </div>
      ...
      {selectMode && (
        <div className="admin-blogs-bulk-actions">
          {STATUS_BULK.map((status) => (
            <button key={status} onClick={() => { setBulkActionType("status"); setBulkActionStatus(status); setBulkConfirmVisible(true); }}>{status}</button>
          ))}
          <button onClick={() => { setBulkActionType("delete"); setBulkConfirmVisible(true); }}>DELETE</button>
        </div>
      )}
                        ...
                        {openStatus && (
                          <div className="admin-header-dropdown-menu">
                            {STATUS_FILTER.map((st) => (
                              <div key={st} className="admin-header-dropdown-item" onClick={() => { setSelectedStatus(st); setOpenStatus(false); }}>{st}</div>
                            ))}
                          </div>
                        )}
                      </div>
                        ...
                        {openCreatedAt && (
                          <div className="admin-header-dropdown-menu">
                            {["Newest First", "Oldest First"].map((order) => (
                              <div key={order} className="admin-header-dropdown-item" onClick={() => { setCreatedSortOrder(order as "Newest First" | "Oldest First"); setOpenCreatedAt(false); }}>{order}</div>
                            ))}
                          </div>
                        )}
                      </div>
                ...
                <tbody>
                  {loading ? (
                    ...
                    paginatedBlogs.map((blog) => (
                      <tr key={blog.blog_id} className="admin-blogs-table-content" style={{ cursor: selectMode ? "default" : "pointer" }} onClick={() => !selectMode && setSelectedBlog(blog)}>
                        ...
                        <td className="admin-blogs-more-button">
                          {selectMode ? (
                            <input type="checkbox" checked={selectedBlogIds.includes(blog.blog_id)} onChange={(e) => setSelectedBlogIds((prev) => e.target.checked ? [...prev, blog.blog_id] : prev.filter((id) => id !== blog.blog_id))} onClick={(e) => e.stopPropagation()} />
                          ) : (
                            <button onClick={(e) => { e.stopPropagation(); setSelectedBlog(blog); }}>
                              <BsThreeDots />
                            </button>
                          )}
                        </td>
                    ...
                    <tr className="admin-blogs-table-content no-blogs-row">
                      <td colSpan={7} className="no-blogs-message">No Blog Found.</td>
                    </tr>
                  )}
                </tbody>
              </table>
              ...
              <div className="blog-grid-scrollable-wrapper">
                <div className="blog-grid-container">
                  {filteredBlogs.map((blog) => (
                    <div key={blog.blog_id} className={`blog-grid-card grid-status-${blog.blog_status.toLowerCase()}`} onClick={() => setSelectedBlog(blog)}>
              ...
              <div className="admin-blogs-float-buttons">
                {isEditing ? (
                  ...
                  <>
                    ...
                    <button className="delete-btn" onClick={() => setConfirmDeleteVisible(true)}>Delete</button>
                  </>
                )}
              </div>

              <button className="admin-blogs-modal-close" onClick={() => { if (isEditing) handleCancel(); setSelectedBlog(null); }} aria-label="Close">✕</button>

              <div className="admin-blogs-modal-inner-content">
                {notification && (
                  <div className={`blogs-notification-message ${notification.includes("successfully") ? "success" : "error"} show`}>{notification}</div>
                )}
                ...
                {confirmDeleteVisible && (
                  <div className="blogs-confirmation-popup show">
                    ...
                    <div className="blogs-confirmation-actions">
                      <button className="confirm-yes" onClick={confirmDeleteBlog}>Yes</button>
                      <button className="confirm-no" onClick={() => setConfirmDeleteVisible(false)}>No</button>
                    </div>
                  </div>
                )}
                ...
                <div className="admin-blogs-modal-inner-content-top">
                  <div className="admin-blogs-modal-left">
                    ...
                    <div className="admin-blogs-modal-title">
                      <p><strong>Title</strong> {submittedEdit && !editableBlog?.title?.trim() && <span style={{ color: "red" }}>*</span>}</p>
                      {isEditing ? (
                        <input type="text" value={editableBlog?.title || ""} onChange={(e) => editableBlog && setEditableBlog({ ...editableBlog, title: e.target.value })} className="admin-blogs-modal-title-content" />
                      ) : (
                    ...
                    <div className="admin-blogs-modal-category">
                      ...
                      <select className={`admin-blogs-modal-select modal-category-${(isEditing ? editableBlog?.category : selectedBlog.category)?.toLowerCase()}`} value={isEditing ? editableBlog?.category : selectedBlog.category} disabled={!isEditing} onChange={(e) => setEditableBlog({ ...editableBlog!, category: e.target.value })}>
                    ...
                    <div className="admin-blogs-modal-status">
                      ...
                      <select className={`admin-blogs-modal-select modal-status-${(isEditing ? editableBlog?.blog_status : selectedBlog.blog_status)?.toLowerCase()}`} value={isEditing ? editableBlog?.blog_status : selectedBlog.blog_status} disabled={!isEditing} onChange={(e) => { const newStatus = e.target.value; if (newStatus === "PINNED") { const pins = blogs.filter((b) => b.blog_status === "PINNED").length; const wasPinned = editableBlog?.blog_status === "PINNED"; if (pins >= MAX_PINNED && !wasPinned) { showNote("You can only pin up to 3 blogs. Please unpin one first.", "error"); return; } } setEditableBlog({ ...editableBlog!, blog_status: newStatus }); }}>
                  ...
                  <div className="admin-blogs-modal-right">
                    <div className="admin-blogs-modal-image">
                      <p><strong>Main Image</strong> {submittedEdit && !editableBlog?.image_url && <span style={{ color: "red" }}>*</span>}</p>
                      {(isEditing ? editableBlog?.image_url : selectedBlog.image_url) ? (
                        <img src={getFullUrl(isEditing ? editableBlog?.image_url : selectedBlog.image_url)} alt="Blog" style={{ cursor: "zoom-in" }} onClick={() => setFullImageUrl(getFullUrl(isEditing ? editableBlog?.image_url : selectedBlog.image_url))} />
                      ) : (
                      ...
                      <div className="admin-blogs-image-buttons">
                        <button className="upload-btn" disabled={!isEditing} onClick={() => { if (isEditing) document.getElementById("upload-image-input")?.click(); }}>Upload</button>
                        <button className="remove-btn" disabled={!isEditing} onClick={handleImageRemove}>Remove</button>
                      </div>
                    </div>
                            ...
                            if (img) {
                              return (
                                <div key={i} className="blog-image-preview">
                                  <img src={getFullUrl(img)} alt={`More Image ${i}`} onClick={() => setFullImageUrl(getFullUrl(img))} style={{ cursor: "zoom-in" }} />
                                  {isLast && <div className="blog-image-overlay" onClick={() => setShowAllImagesModal(true)}>+{total - 3}</div>}
                                </div>
                              );
                            }
                            return isEditing ? (
                              <label key={i} htmlFor="edit-more-images-input" className="blog-more-image-placeholder-cell clickable"><span className="blog-placeholder-icon">+</span></label>
                            ) : (
                              <div key={i} className="blog-more-image-placeholder-cell"><span className="blog-placeholder-icon">+</span></div>
                            );
                          })}
                        </div>
                ...
                <div className="admin-blogs-modal-inner-content-bot">
                  <div className="admin-events-inner-content-modal-desc">
                    <p><strong>Blog Content</strong> {submittedEdit && !rawText && <span style={{ color: "red" }}>*</span>}</p>
                    {isEditing ? (
                      <>
                        <div className="admin-blogs-content-image-tools">
                          <button className="format-btn undo" onMouseDown={(e) => e.preventDefault()} onClick={() => document.execCommand("undo", false)}><FaUndo /></button>
                          <button className="format-btn redo" onMouseDown={(e) => e.preventDefault()} onClick={() => document.execCommand("redo", false)}><FaRedo /></button>
                          <button className="format-btn bold" onMouseDown={(e) => { e.preventDefault(); saveSelection(); }} onClick={() => applyFormatting("bold")}><FaBold /></button>
                          ...
                          <input type="file" accept="image/*" id="edit-content-image-input" style={{ display: "none" }} onChange={async (e) => {
                            ...
                            try {
                              ...
                              if (data.success && data.image_url) {
                                ...
                                if (div) { div.innerHTML += img; setEditableBlog((prev) => (prev ? { ...prev, content: div.innerHTML } : prev)); }
                              } else { toast.error("Image upload failed."); }
                            } catch (err) { console.error("Upload failed:", err); toast.error("An error occurred during upload."); }
                          }} />
                        </div>
                        <div id="new-blog-content-editor" ref={textareaRef} className="admin-blogs-modal-desc-content editable" contentEditable onBlur={() => { if (textareaRef.current) { const html = textareaRef.current.innerHTML; setEditableBlog((prev) => (prev ? { ...prev, content: html } : prev)); } }} />
                      </>
                    ) : (
          ...
          <div className="admin-blogs-new-blog-modal">
            <div className="admin-blogs-new-blog-modal-content">
              <div className="admin-blogs-new-blog-float-buttons">
                ...
                <button className="cancel-btn" onClick={() => { resetNewBlogForm(); setNewBlogMoreImages([]); setShowAllImagesModal(false); setNewBlogModalOpen(false); }}>Cancel</button>
              </div>

              <button className="admin-blogs-new-blog-modal-close" onClick={() => { resetNewBlogForm(); setNewBlogMoreImages([]); setShowAllImagesModal(false); setNewBlogModalOpen(false); }} aria-label="Close">✕</button>
              ...
              <div className="admin-blogs-new-blog-modal-inner-content">
                <div className="admin-blogs-new-blog-modal-inner-content-top">
                  <div className="admin-blogs-new-blog-modal-left">
                    ...
                    <div className="admin-blogs-new-blog-modal-title">
                      <p><strong>Title {!newBlogTitle.trim() && <span style={{ color: "red" }}>*</span>}</strong></p>
                      <input type="text" className="admin-blogs-new-blog-modal-title-content" value={newBlogTitle} onChange={(e) => setNewBlogTitle(e.target.value)} placeholder="Enter title" />
                    </div>

                    <div className="admin-blogs-new-blog-modal-category">
                      <p><strong>Category {!newBlogCategory.trim() && <span style={{ color: "red" }}>*</span>}</strong></p>
                      <select className="admin-blogs-new-blog-modal-select modal-category-pink" value={newBlogCategory} onChange={(e) => setNewBlogCategory(e.target.value as (typeof NEW_CATEGORIES)[number])}>
                        {NEW_CATEGORIES.map((cat) => <option key={cat} value={cat}>{cat}</option>)}
                      </select>
                    </div>
                    ...
                    <div className="admin-blogs-new-blog-modal-status">
                      <p><strong>Status {!newBlogStatus.trim() && <span style={{ color: "red" }}>*</span>}</strong></p>
                      <select className={`admin-blogs-new-blog-modal-select modal-status-${newBlogStatus.toLowerCase()}`} value={newBlogStatus} onChange={(e) => setNewBlogStatus(e.target.value as "DRAFT" | "PUBLISHED")}>
                        {["DRAFT", "PUBLISHED"].map((st) => <option key={st} value={st}>{st}</option>)}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="admin-blogs-new-blog-modal-right">
                    <div className="admin-blogs-new-blog-modal-image">
                      <p><strong>Main Image {!newBlogImage && <span style={{ color: "red" }}>*</span>}</strong></p>
                      {newBlogImage ? (
                        <img src={getFullUrl(newBlogImage)} alt="Preview" style={{ maxWidth: "100%", maxHeight: "300px", borderRadius: "4px", cursor: "zoom-in" }} onClick={() => setFullImageUrl(getFullUrl(newBlogImage))} />
                      ) : (
                        <div className="no-image-placeholder">No Blog Image</div>
                      )}
                      <input type="file" accept="image/*" style={{ display: "none" }} id="new-blog-image-input" onChange={(e) => handleImageSelect(e, "new")} />
                      <div className="admin-blogs-image-buttons">
                        <button className="upload-btn" onClick={() => document.getElementById("new-blog-image-input")?.click()}>Upload</button>
                        <button className="remove-btn" onClick={() => setNewBlogImage("")}>Remove</button>
                      </div>
                    </div>
                      ...
                      <div className="blog-more-image-grid">
                        {[...Array(4)].map((_, i) => {
                          ...
                          if (image) {
                            return (
                              <div key={i} className="blog-image-preview">
                                <img src={getFullUrl(image)} alt={`More Image ${i}`} onClick={() => setFullImageUrl(getFullUrl(image))} style={{ cursor: "zoom-in" }} />
                                {isLast && <div className="blog-image-overlay" onClick={() => setShowAllImagesModal(true)}>+{total - 3}</div>}
                              </div>
                            );
                          }
                          return <label key={i} htmlFor="new-blog-more-images-input" className="blog-more-image-placeholder-cell clickable"><span className="blog-placeholder-icon">+</span></label>;
                        })}
                      </div>

                      <div className="edit-more-images-buttons">
                        <button className="upload-btn" onClick={() => document.getElementById("new-blog-more-images-input")?.click()}>Add More</button>
                ...
                <div className="admin-blogs-new-blog-modal-inner-content-bot">
                  <div className="admin-blogs-new-blog-modal-desc">
                    <p><strong>Blog Content {!newBlogContent.trim() && <span style={{ color: "red" }}>*</span>}</strong></p>
                    <div className="admin-blogs-new-blog-modal-desc">
                      <div className="admin-blogs-content-image-tools">
                        <button className="format-btn undo" onMouseDown={(e) => e.preventDefault()} onClick={() => document.execCommand("undo")}><FaUndo /></button>
                        <button className="format-btn redo" onMouseDown={(e) => e.preventDefault()} onClick={() => document.execCommand("redo")}><FaRedo /></button>
                        <button className="format-btn bold" onMouseDown={(e) => e.preventDefault()} onClick={() => document.execCommand("bold")}><FaBold /></button>
                        <button className="format-btn italic" onMouseDown={(e) => e.preventDefault()} onClick={() => document.execCommand("italic")}><FaItalic /></button>
                        ...
                        <input type="file" accept="image/*" id="new-content-image-input" style={{ display: "none" }} onChange={async (e) => {
                          ...
                          try {
                            ...
                            if (data.success && data.image_url) {
                              ...
                              const div = document.getElementById("new-blog-content-editor");
                              if (div) div.innerHTML += img; setNewBlogContent((prev) => prev + img);
                            } else { toast.error("Image upload failed."); }
                          } catch (err) { console.error(err); toast.error("Upload failed."); }
                        }} />
                      </div>
                      <div id="new-blog-content-editor" className="admin-blogs-new-blog-modal-desc-content editable" contentEditable onBlur={(e) => setNewBlogContent(e.currentTarget.innerHTML)} dangerouslySetInnerHTML={{ __html: newBlogContent }} />
                    </div>
                  </div>
                </div>
              </div>
      ...
      {showAllImagesModal && (
        <div className="blog-gallery-modal">
          ...
          <div className="blog-gallery-wrapper">
            <button className="blog-gallery-close" onClick={() => setShowAllImagesModal(false)} aria-label="Close">✕</button>
                  ...
                  <div className="thumb-image-wrapper" onClick={() => setFullImageUrl(getFullUrl(img))}>
                    ...
                    {(isEditing || newBlogModalOpen) && (
                      <div className="thumb-controls">
                        {index > 0 && (
                          <button className="thumb-swap-left" onClick={(e) => { e.stopPropagation(); const updater = isEditing ? setEditableBlogMoreImages : setNewBlogMoreImages; updater((prev) => { const u = [...prev]; [u[index - 1], u[index]] = [u[index], u[index - 1]]; return u; }); }}>←</button>
                        )}
                        <button className="thumb-delete" onClick={(e) => { e.stopPropagation(); setConfirmThumbDeleteIndex(index); const updater = isEditing ? setEditableBlogMoreImages : setNewBlogMoreImages; updater((prev) => prev.filter((_, i) => i !== index)); }}>🗑</button>
                        {index < imageList.length - 1 && (
                          <button className="thumb-swap-right" onClick={(e) => { e.stopPropagation(); const updater = isEditing ? setEditableBlogMoreImages : setNewBlogMoreImages; updater((prev) => { const u = [...prev]; [u[index], u[index + 1]] = [u[index + 1], u[index]]; return u; }); }}>→</button>
                        )}
                      </div>
                    )}
                  </div>
      ...
      {fullImageUrl && (
        <div className="blog-fullscreen-viewer">
          ...
          <button className="blog-fullscreen-exit" onClick={() => setFullImageUrl("")} aria-label="Close">✕</button>
        </div>
      )}
      ...
      {bulkConfirmVisible && (
        <div className="blogs-confirmation-popup show">
          <div className="blogs-confirmation-box">
            <p>{bulkActionType === "delete" ? "Are you sure you want to delete the selected blogs?" : `Do you really want to mark the selected blogs as ${bulkActionStatus}?`}</p>
            <div className="blogs-confirmation-actions">
              <button className="confirm-yes" onClick={() => { if (bulkActionType === "delete") { handleBulkDelete(); } else { applyBulkStatus(bulkActionStatus); } setBulkConfirmVisible(false); }}>Yes</button>
